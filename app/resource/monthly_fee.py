...
class MonthlyFee(Resource):
    @ns.doc('create')
    @ns.expect(monthly_fee_schema)
    @ns.marshal_with(monthly_fee_schema, code=HTTPStatus.CREATED)
    ...
    @ns.doc('get_all')
    @ns.marshal_list_with(monthly_fee_schema)
    def get(self):
...
class MonthlyFeeById(Resource):
    @ns.doc('get_one')
    @ns.marshal_with(monthly_fee_schema)
    def get(self, id: int):
...
@ns.param('name', 'The monthly fee name')
